...
- Importing a macro also converts `/cancelaura` lines: the aura becomes `{AURA}` (`{AURA_2}`...) regardless of its category. `{AURA}` and `{CANCEL}` get their own **Cancel aura** category: per class, the auras that stop you from acting (Deep Breath, Ice Block, Aspect of the Turtle, Divine Shield, Bladestorm, Ghost Wolf...), not defensives. Optional placeholders are never pre-filled.
