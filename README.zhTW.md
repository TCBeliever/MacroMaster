...
內附 25 個模板。全新安裝時清單只有你這個職業的推薦組（約八個），其餘在「設定 → 預設模板」依分類列出，適合你職業的排前面。清單完全是你的：改、刪、加都可以。沒改過的內建模板更新時會自動跟上新版，改過的不動。
...
- 編輯區的「分享」會把目前的模板變成一串 `!MM1!...` 字串。「設定 → 匯出／匯入 → 匯出」則是所有模板一串，適合備份。
- 匯入：把字串（或舊版的純文字）貼進同一個視窗按「匯入」。「同 id」決定遇到你已經有的模板怎麼辦：「取代」覆蓋你的那份（還原備份用），「另外新增」並排加一份（保留你的）。
...
- **預設模板**：內附模板的唯讀目錄。可「加入我的模板」、用預設覆蓋你的版本、或把整份清單還原成職業推薦組。
- **匯出／匯入**：見上一節。
